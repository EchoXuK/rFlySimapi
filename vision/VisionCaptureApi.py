...
import warnings
# warnings.simplefilter("always")  # 打开所有警告
# warnings.filterwarnings("error", category=UserWarning)  # 将 UserWarning 转为异常
warnings.filterwarnings("ignore", category=UserWarning)  # 将 UserWarning 转为异常
...
class VisionCaptureApi:
    ...
    def __del__(self):
        ...
        if isEnableRosTrans and not is_use_ros1:
            ...
            rclpy.shutdown()
    def euler2quat(self,r,p,y,copter:str,coordinate_frame="global"): #使用这个函数得保证旋转旋转循序为绕z,y,x轴
        if isEnableRosTrans:
            roll = r
            pitch = p
            yaw = y
            if(coordinate_frame == "local"):
                roll = r - self.initPose[copter][3]
                pitch = p - self.initPose[copter][4]
                yaw = y - self.initPose[copter][5]
    ...
    def __init__(self, ip="127.0.0.1"):
        ...
        self.initPose = {} #初始化所有飞机默认在地图里的位置，给local 的 odom 使用
                ...
                if dataOk:  # 如果数据都没问题，开始处理图像
                    ...
                    if (
                        ...
                        if self.VisSensor[idx].SendProtocol[0] == 1 and (
                            ...
                            copterid = np.frombuffer(data_total, dtype=np.int32,count=1)[0]
                            axis_type=np.frombuffer(data_total, dtype=np.int32,count=1,offset= 4)[0]
                            posAng = np.frombuffer(
                                data_total, dtype=np.float32, count=6 ,offset=4 * 2
                            )  # pos ang  6*4
                            if isEnableRosTrans:
                                global_key = "global" + str(copterid)
                                local_key = "local" + str(copterid)
                                if(not global_key in self.initPose.keys()):
                                    self.initPose[global_key] = np.array(posAng) #初始化飞机的位姿
                            # print("copterid ",copterid)
                            # print("axis_type",axis_type)

                            PointNum = np.frombuffer(
                                data_total, dtype=np.int32, count=1, offset=4 * 8
                            )  # num 4*1/
                            PointNum = PointNum[0]
                            # print('PointNum: ',PointNum)
                            # print('posAng: ', posAng)
                            self.ImgData[idx] =  [copterid] + [axis_type] + posAng.tolist() + [PointNum]
                            ...
                            L = np.frombuffer(
                                ...
                                * 4,  # --Lidar PointNum * 3, Fix PointNum * 4
                                offset=4 * 9,
                            )  # clou                            data_total = data_total + dataList[idx0]                            data_total = data_total + dataList[idx0]d
                    ...
                    if isEnableRosTrans and self.hasData[idx]:  # 如果需要发布ros消息
                        ...
                        if type_id == 20 or type_id == 21 or type_id == 22 or type_id == 23:
                            ...
                            if not local_key in self.topic_name.keys():
                                self.topic_name[local_key] = ("/rflysim/uav" + str(copterid)+"/local" + "/odom")
                            if not global_key in self.topic_name.keys():
                                self.topic_name[global_key] = ("/rflysim/uav" + str(copterid)+"/global" + "/odom")
                            ...
                            # print("pos ang ",posAng)
                            #发布飞机位置话题
                            if not global_key in self.sensor_data.keys():
                                msg = Odometry()
                                msg.header.frame_id = "odom" + str(copterid)
                                msg.child_frame_id = "base_link" + str(copterid)
                                self.sensor_data[global_key] = msg
                            if not local_key in self.sensor_data.keys():
                                msg = Odometry()
                                msg.header.frame_id = "odom" + str(copterid)
                                msg.child_frame_id = "base_link" + str(copterid)
                                self.sensor_data[local_key] = msg 
                        ...
                        if(len(posAng) == 6):
                            self.sensor_data[global_key].pose.pose.position.x = posAng[0]
                            self.sensor_data[global_key].pose.pose.position.y = posAng[1]
                            self.sensor_data[global_key].pose.pose.position.z = posAng[2]
                            self.sensor_data[global_key].pose.pose.orientation = self.euler2quat(posAng[3],posAng[4],posAng[5],global_key)
                            self.sensor_data[local_key].pose.pose.position.x = posAng[0]-self.initPose[global_key][0]
                            self.sensor_data[local_key].pose.pose.position.y = posAng[1] - self.initPose[global_key][1]
                            self.sensor_data[local_key].pose.pose.position.z = posAng[2] -self.initPose[global_key][2]
                            self.sensor_data[local_key].pose.pose.orientation = self.euler2quat(posAng[3],posAng[4],posAng[5],global_key,"local")
                        ...
                        if is_use_ros1:
                            ...
                            if not global_key in  self.topic_name.keys(): #如果话题名还没创建，就没有必要创建发布器
                                continue
                            if not local_key in  self.topic_name.keys(): #如果话题名还没创建，就没有必要创建发布器
                                continue
                            
                            #发布global下的位姿
                            if not self.topic_name[global_key] in self.sensor_pub.keys():
                                self.sensor_pub[
                                    self.topic_name[global_key]
                                    ]=rospy.Publisher(self.topic_name[global_key],Odometry,queue_size=10)
                            self.sensor_data[global_key].header.stamp = rospy.Duration(
                                self.imgStmp[idx]
                            )
                            self.sensor_pub[self.topic_name[global_key]].publish(
                                self.sensor_data[global_key]
                            )
                            
                            #发布local下的位姿
                            if not self.topic_name[local_key] in self.sensor_pub.keys():
                                self.sensor_pub[
                                    self.topic_name[local_key]
                                    ]=rospy.Publisher(self.topic_name[local_key],Odometry,queue_size=10)
                            self.sensor_data[local_key].header.stamp = rospy.Duration(
                                self.imgStmp[idx]
                            )
                            self.sensor_pub[self.topic_name[local_key]].publish(    
                                self.sensor_data[local_key]
                            )
                        else: #ROS2 发布
                            if not self.topic_name[seq_id] in self.sensor_pub.keys():
                                self.sensor_pub[
                            ...
                            if not self.topic_name[global_key] in self.sensor_pub.keys():
                                self.sensor_pub[
                                    ...
                                    type(self.sensor_data[global_key]),
                                    self.topic_name[global_key],
                                    qos_profile=qos_profile_sensor_data,
                                )
                            ...
                            if not self.topic_name[local_key] in self.sensor_pub.keys():
                                self.sensor_pub[
                                    ...
                                    type(self.sensor_data[local_key]),
                                    self.topic_name[local_key],
                                    qos_profile=qos_profile_sensor_data,
                                )
                            ...
                            self.sensor_data[global_key].header.stamp.sec = int(seconds & 0x7FFFFFFF)
                            self.sensor_data[global_key].header.stamp.nanosec = nanoseconds
                            self.sensor_pub[self.topic_name[global_key]].publish(self.sensor_data[global_key])
                            
                            self.sensor_data[local_key].header.stamp.sec = int(seconds & 0x7FFFFFFF)
                            self.sensor_data[local_key].header.stamp.nanosec = nanoseconds
                            self.sensor_pub[self.topic_name[local_key]].publish(self.sensor_data[local_key])
        udpSok.close()
                ...
                for ii in range(3):  # 尝试读取三次内存区域
                    ...
                    if flag[0] == 2:  # 图像已写入完成
                        ...
                        if (
                            ...
                            self.Img_lock[idx].release()
                            self.hasData[idx] = True
                            self.timeStmp[idx] = timeStmpStore # type: ignore
